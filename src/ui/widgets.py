...
from PyQt6.QtGui import QPainter, QColor, QBrush, QPen, QFont, QDrag, QMouseEvent, QPaintEvent, QWheelEvent, QDragEnterEvent, QDropEvent
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if item:
                ...
                if tid is not None:
                    drag = QDrag(self)
                    mime = QMimeData(); mime.setText(str(tid)); drag.setMimeData(mime)
...
class LibraryWaveformPreview(QWidget):
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if e - s < 0.01:
                self.selection_start = None; self.selection_end = None; self.selectionChanged.emit(0.0, 1.0)
            else:
                self.selection_start = s; self.selection_end = e; self.selectionChanged.emit(s, e)
            self.update()
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        if self.selection_start is not None and self.selection_end is not None:
            ...
            p.drawLine(s, 0, s, self.height()); p.drawLine(e, 0, e, self.height())
...
class LoadingOverlay(QWidget):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.message_label.setAlignment(Qt.AlignmentFlag.AlignCenter); self.layout.addWidget(self.message_label)
        self.progress_bar = QProgressBar(); self.progress_bar.setFixedWidth(400)
    ...
    def show_loading(self, m: str = "Processing...", total: int = 0) -> None:
        self.message_label.setText(m); self.progress_bar.setValue(0)
        if total > 0: self.progress_bar.setRange(0, total)
        else: self.progress_bar.setRange(0, 0)
        if self.parentWidget(): self.setGeometry(self.parentWidget().rect())
        self.raise_(); self.show(); QApplication.processEvents()
...
class TimelineWidget(QWidget):
    ...
    aiTransitionRequested = pyqtSignal(float, str) 
    duplicateRequested = pyqtSignal(object) 
    captureRequested = pyqtSignal(object) 
    stemsRequested = pyqtSignal(object) 
    sidechainRequested = pyqtSignal(object, int) 
    zoomChanged = pyqtSignal(int)
    trackDropped = pyqtSignal(object, int, int) 
    fillRangeRequested = pyqtSignal(float, float) 

    def __init__(self) -> None:
        super().__init__()
        self.segments: List[TrackSegment] = []
        self.setMinimumHeight(550); self.setAcceptDrops(True); self.setMouseTracking(True)
        self.pixels_per_ms: float = 0.05; self.selected_segment: Optional[TrackSegment] = None
        ...
        self.keyframe_dragging: bool = False; self.selected_keyframe_idx: int = -1; self.selected_keyframe_param: Optional[str] = None; self.active_automation_param: str = "volume"
        self.drag_start_pos: Optional[QPoint] = None; self.drag_start_ms = self.drag_start_dur = self.drag_start_fade = self.drag_start_offset = 0.0
        self.drag_start_vol: float = 1.0; self.drag_start_lane: int = 0; self.lane_height: int = 120; self.lane_spacing: int = 10; self.snap_threshold_ms: float = 2000.0
        self.target_bpm: float = 124.0; self.show_modifications: bool = True; self.cursor_pos_ms: float = 0.0; self.show_waveforms: bool = True; self.snap_to_grid: bool = True
        self.lane_count: int = 20; self.mutes: List[bool] = [False] * self.lane_count; self.solos: List[bool] = [False] * self.lane_count
        self.loop_start_ms: float = 0.0; self.loop_end_ms: float = 30000.0; self.loop_enabled: bool = False
        self.scorer: CompatibilityScorer = CompatibilityScorer(); self.silence_regions: List[Tuple[float, float]] = []
        self.update_geometry()

    def add_lane(self) -> None:
        self.lane_count += 1; self.mutes.append(False); self.solos.append(False); self.update_geometry(); self.timelineChanged.emit()

    def remove_lane(self) -> None:
        if self.lane_count > 1:
            self.lane_count -= 1; self.mutes.pop(); self.solos.pop()
            for s in self.segments:
                if s.lane >= self.lane_count: s.lane = self.lane_count - 1
            self.update_geometry(); self.timelineChanged.emit()

    def find_silence_regions(self) -> List[Tuple[float, float]]:
        if not self.segments: return []
        total_len = max(s.start_ms + s.duration_ms for s in self.segments)
        gaps: List[Tuple[float, float]] = []; step_ms = 500; threshold = 0.15; in_gap = False; gap_start = 0.0
        for t in range(0, int(total_len), step_ms):
            combined_vol = 0.0
            for s in self.segments:
                if s.start_ms <= t <= (s.start_ms + s.duration_ms):
                    seg_v = s.volume; rel_t = t - s.start_ms
                    if rel_t < s.fade_in_ms and s.fade_in_ms > 0: seg_v *= (rel_t / s.fade_in_ms)
                    elif rel_t > (s.duration_ms - s.fade_out_ms) and s.fade_out_ms > 0: seg_v *= ((s.duration_ms - rel_t) / s.fade_out_ms)
                    combined_vol += seg_v
            if combined_vol < threshold:
                if not in_gap: in_gap = True; gap_start = float(t)
            else:
                if in_gap:
                    in_gap = False
                    if t - gap_start > 500: gaps.append((gap_start, float(t)))
    ...
    def update_geometry(self) -> None:
        max_ms = 600000.0
        if self.segments: max_ms = max(max_ms, max(s.start_ms + s.duration_ms for s in self.segments) + 60000.0)
    ...
    def get_ms_per_beat(self) -> float:
        return (60.0 / self.target_bpm) * 1000.0

    def get_seg_rect(self, seg: TrackSegment) -> QRect:
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        for start, end in self.silence_regions:
            ...
            painter.fillRect(sx, 0, sw, 40, QColor(255, 50, 50, 80)); painter.setPen(QPen(QColor(255, 50, 50, 150), 1)); painter.drawText(sx + 2, 38, "⚠ GAP")
        if self.loop_enabled:
            lx = int(self.loop_start_ms * self.pixels_per_ms); lw = int((self.loop_end_ms - self.loop_start_ms) * self.pixels_per_ms)
            painter.fillRect(lx, 0, lw, 40, QColor(0, 200, 255, 60)); painter.setPen(QPen(QColor(0, 200, 255, 150), 2))
            painter.drawLine(lx, 0, lx, 40); painter.drawLine(lx + lw, 0, lx + lw, 40)
        ...
        for i in range(self.lane_count): 
            ...
            painter.setPen(QColor(150, 150, 150)); painter.drawText(5, y + 15, f"LANE {i+1}")
            mr = QRect(5, y + 25, 20, 20); painter.setBrush(QBrush(QColor(255, 50, 50) if self.mutes[i] else QColor(60, 60, 60)))
            painter.setPen(Qt.PenStyle.NoPen); painter.drawRoundedRect(mr, 3, 3); painter.setPen(Qt.GlobalColor.white); painter.drawText(mr, Qt.AlignmentFlag.AlignCenter, "M")
            sr = QRect(30, y + 25, 20, 20); painter.setBrush(QBrush(QColor(255, 200, 0) if self.solos[i] else QColor(60, 60, 60)))
            painter.setPen(Qt.PenStyle.NoPen); painter.drawRoundedRect(sr, 3, 3); painter.setPen(Qt.GlobalColor.white); painter.drawText(sr, Qt.AlignmentFlag.AlignCenter, "S")
        mpb = self.get_ms_per_beat(); mpbar = mpb * 4
        for i in range(0, 3600000, int(mpb)):
            x = int(i * self.pixels_per_ms); if x > self.width(): break
            if (i % int(mpbar)) < 10: painter.setPen(QPen(QColor(80, 80, 80), 1)); painter.drawLine(x, 0, x, self.height()); painter.setPen(QColor(150, 150, 150)); painter.drawText(x + 5, 15, f"BAR {int(i // mpbar) + 1}")
        ...
        for s in range(0, 3600, 10):
            ms = s * 1000; x = int(ms * self.pixels_per_ms); if x > self.width(): break
            painter.drawLine(x, 25, x, 40)
            if s % 30 == 0: mins = s // 60; secs = s % 60; painter.setPen(QColor(0, 200, 255, 180)); painter.drawText(x + 5, 35, f"{mins}:{secs:02d}"); painter.setPen(QPen(QColor(0, 200, 255, 100), 1))
        for seg in self.segments:
            rect = self.get_seg_rect(seg); color = QColor(seg.color); is_ducked = False
            if not seg.is_primary:
                for o in self.segments:
                    if o != seg and o.is_primary and max(seg.start_ms, o.start_ms) < min(seg.get_end_ms(), o.get_end_ms()): is_ducked = True; break
            hc = False
            for o in self.segments:
            ...
            if self.show_waveforms:
                if hasattr(seg, 'stem_waveforms') and seg.stem_waveforms:
                    stems = [("vocals", QColor(255, 204, 0, 180)), ("drums", QColor(255, 51, 102, 180)), ("bass", QColor(0, 204, 255, 180)), ("other", QColor(153, 51, 255, 180))]
                    stem_h = rect.height() // 4
                    for idx, (stype, scolor) in enumerate(stems):
                        if stype in seg.stem_waveforms:
                            sw = seg.stem_waveforms[stype]; painter.setPen(QPen(scolor, 1)); pts = len(sw); s_mid = rect.top() + (idx * stem_h) + (stem_h // 2); s_max_h = stem_h // 2 - 2
                            for i in range(0, rect.width(), 2):
                                ri = (i / rect.width()) * (seg.duration_ms / 30000.0); s_idx = int((ri + (seg.offset_ms / 30000.0)) * pts) % pts
                                val = sw[s_idx] * s_max_h; painter.drawLine(rect.left() + i, int(s_mid - val), rect.left() + i, int(s_mid + val))
                elif seg.waveform:
                    painter.setPen(QPen(QColor(255, 255, 255, 80), 1)); pts = len(seg.waveform); mid_y = rect.center().y(); max_h = rect.height() // 2
                    for i in range(0, rect.width(), 2):
                        ri = (i / rect.width()) * (seg.duration_ms / 30000.0); idx = int((ri + (seg.offset_ms / 30000.0)) * pts) % pts
                        val = seg.waveform[idx] * max_h; painter.drawLine(rect.left() + i, int(mid_y - val), rect.left() + i, int(mid_y + val))
            ...
            if hasattr(seg, 'sections') and seg.sections:
                ...
                for sec in seg.sections:
                    s_ms = sec['start'] * 1000.0; adj = (s_ms - seg.offset_ms) * s_f
                    if 0 <= adj <= seg.duration_ms:
                        tx = rect.left() + int(adj * self.pixels_per_ms); label = sec['label'].upper()
                        ...
                        painter.setPen(QPen(s_color, 1, Qt.PenStyle.DashLine)); painter.drawLine(tx, rect.top(), tx, rect.bottom())
                        painter.setPen(s_color); painter.setFont(QFont("Segoe UI", 7, QFont.Weight.Bold)); painter.drawText(tx + 3, rect.bottom() - 5, label)
            ...
            if hasattr(seg, 'reverb') and seg.reverb > 0: painter.setBrush(QBrush(QColor(0, 200, 255, int(255 * seg.reverb)))); painter.drawEllipse(rect.right() - 25, rect.bottom() - 25, 12, 12)
            if hasattr(seg, 'harmonics') and seg.harmonics > 0: painter.setBrush(QBrush(QColor(255, 150, 0, int(255 * seg.harmonics)))); painter.drawEllipse(rect.right() - 45, rect.bottom() - 25, 12, 12)
            if hasattr(seg, 'keyframes') and self.active_automation_param in seg.keyframes:
                pts = seg.keyframes[self.active_automation_param]
                if pts:
                    if self.active_automation_param == "volume": k_color = QColor(255, 200, 0, 200)
                    elif self.active_automation_param == "pan": k_color = QColor(0, 200, 255, 200)
                    elif "cut" in self.active_automation_param: k_color = QColor(0, 255, 100, 200)
                    else: k_color = QColor(255, 100, 255, 200)
                    painter.setPen(QPen(k_color, 2)); painter.setBrush(QBrush(k_color)); sorted_pts = sorted(pts, key=lambda x: x[0])
                    prev_x = rect.left(); prev_y = rect.bottom() - int(rect.height() * sorted_pts[0][1])
                    for ms, val in sorted_pts:
                        x = rect.left() + int(ms * self.pixels_per_ms); y = rect.bottom() - int(rect.height() * max(0.0, min(1.0, val)))
                        painter.drawLine(prev_x, prev_y, x, y); painter.drawEllipse(x - 3, y - 3, 6, 6); prev_x = x; prev_y = y
                    if prev_x < rect.right(): painter.drawLine(prev_x, prev_y, rect.right(), prev_y)
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if ts:
                ...
                elif act == sl: self.captureRequested.emit(ts)
                elif act in pm.actions(): self.undoRequested.emit(); ts.pitch_shift = int(act.data())
                elif act == da_rem: self.undoRequested.emit(); self.segments.remove(ts)
                elif act == ra_keys: self.undoRequested.emit(); ts.keyframes = {}
                elif act in scm.actions(): self.sidechainRequested.emit(ts, int(act.data()))
                if self.selected_segment == ts: self.selected_segment = None
                self.update_geometry(); self.timelineChanged.emit()
            else:
                ba = m.addAction("🪄 Find Bridge Track here"); ai_menu = m.addMenu("✨ Generate AI Asset"); ar = ai_menu.addAction("📈 Cinematic Riser"); ad = ai_menu.addAction("📉 Bass Drop"); ap = ai_menu.addAction("🌌 Ambient Pad"); ab = ai_menu.addAction("🥁 Percussion Build")
                fa = fs = fe = None
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self.slipping: self.selected_segment.offset_ms = max(0.0, self.drag_start_offset - dx/self.pixels_per_ms)
            ...
            if self.drag_start_dur - actual_delta > 500: self.selected_segment.start_ms = int(self.drag_start_ms + actual_delta); self.selected_segment.offset_ms = max(0.0, self.drag_start_offset + actual_delta); self.selected_segment.duration_ms = int(self.drag_start_dur - actual_delta)
        elif self.fade_in_dragging: rf = self.drag_start_fade + dx/self.pixels_per_ms; if self.snap_to_grid: rf = round(rf / mpb) * mpb; self.selected_segment.fade_in_ms = int(max(0.0, min(self.selected_segment.duration_ms/2, rf)))
        elif self.fade_out_dragging: rf = self.drag_start_fade - dx/self.pixels_per_ms; if self.snap_to_grid: rf = round(rf / mpb) * mpb; self.selected_segment.fade_out_ms = int(max(0.0, min(self.selected_segment.duration_ms/2, rf)))
        elif self.resizing: rd = self.drag_start_dur + dx/self.pixels_per_ms; if self.snap_to_grid: rd = round(rd / mpb) * mpb; self.selected_segment.duration_ms = int(max(1000.0, rd))
        elif self.vol_dragging: self.selected_segment.volume = max(0.0, min(1.5, self.drag_start_vol - dy/150.0))
        elif self.dragging:
            ns = max(0.0, self.drag_start_ms + dx/self.pixels_per_ms); if self.snap_to_grid: ns = round(ns / mpb) * mpb
            for o in self.segments:
    ...
    def split_segment(self, seg: TrackSegment, x_pos: float) -> None:
        ...
        nd = seg.duration_ms - rs; no = seg.offset_ms + rs; seg.duration_ms = int(rs) 
        ...
        ns = TrackSegment(td, start_ms=int(sm), duration_ms=int(nd), lane=seg.lane, offset_ms=no); ns.volume = seg.volume; ns.is_primary = seg.is_primary; ns.waveform = seg.waveform; ns.pitch_shift = seg.pitch_shift; self.segments.append(ns); self.update_geometry(); self.timelineChanged.emit()

    def quantize_segment(self, seg: TrackSegment) -> None:
        if not seg.onsets: return
        mpb = self.get_ms_per_beat(); stretch = self.target_bpm / seg.bpm; foc = (seg.onsets[0] - seg.offset_ms) * stretch; cwp = seg.start_ms + foc; twp = round(cwp / mpb) * mpb; seg.start_ms += int(twp - cwp); self.update(); self.timelineChanged.emit()

    def add_track(self, td: Dict[str, Any], start_ms: Optional[int] = None, lane: int = 0) -> TrackSegment:
        if start_ms is None:
            if self.segments: l_s = max(self.segments, key=lambda s: s.get_end_ms()); start_ms = l_s.get_end_ms() - 5000; lane = l_s.lane
            else: start_ms = 0
